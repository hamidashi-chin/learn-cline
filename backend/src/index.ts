...
import cors from '@fastify/cors'; // @fastify/cors をインポート
...
fastify.post('/transactions', async (request, reply) => {
  const { date, item, amount, type } = request.body as any; // 型アサーションは一時的なもの
  try {
...
fastify.put('/transactions/:id', async (request, reply) => {
  const { id } = request.params as any; // 型アサーションは一時的なもの
  const { date, item, amount, type } = request.body as any; // 型アサーションは一時的なもの
  try {
...
fastify.get('/summary', async (request, reply) => {
  try {
    ...
    const dailySummary: { [key: string]: { income: number; expense: number } } = {};
    const monthlySummary: { [key: string]: { income: number; expense: number } } = {};
    const yearlySummary: { [key: string]: { income: number; expense: number } } = {};
    ...
    transactions.forEach(transaction => {
      ...
      if (!monthlySummary[yearMonth]) monthlySummary[yearMonth] = { income: 0, expense: 0 };
      if (!yearlySummary[year]) yearlySummary[year] = { income: 0, expense: 0 };
